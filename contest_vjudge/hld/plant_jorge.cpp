...
int main() {
    ...
    G.build();
    vector<ll> hldValues(nodos);
    for (ll i = 1; i <= nodos; i++) {
        hldValues[G.pos[i] - 1] = vec2[i - 1];
    }

    st.build(hldValues); // Inicializacion de segment tree      // LLENO LOS ARREGLOS NECESARIO DE HEAVY LIGHT
    for (ll i = 1; i <= nodos; i++) {
        st.update(G.pos[i], G.pos[i], 10 * i); // En la pos G.pos[i] tengo el valor 10*i
                                               // correspondiente al valor del nodo i
    }
    for (ll i = 1; i <= nodos; i++) {
        cout << "Posicion de nodo en segment tree: " << i << " : " << G.pos[i]
             << " valor: " << st.query(G.pos[i], G.pos[i]) << endl;
    }
    return 0;
}
